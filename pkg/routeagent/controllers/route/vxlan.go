...
func createVxLanIface(iface *vxLanIface) error {
	...
	if err == syscall.EEXIST {
		...
		if isVxlanConfigTheSame(iface.link, existing) {
			klog.V(log.DEBUG_LEVEL).Infof("VxLAN interface already exists with same configuration.")
...
func isVxlanConfigTheSame(new, current netlink.Link) bool {
	...
	if required.Port > 0 && existing.Port > 0 && required.Port != existing.Port {
		klog.V(log.DEBUG_LEVEL).Infof("Vxlan Port (%d) of existing interface does not match with required Port (%d)", existing.Port, required.Port)
		return false
	}
...
func (iface *vxLanIface) AddFDB(ipAddress net.IP, hwAddr string) error {
	...
	if err != nil {
		...
		klog.V(log.DEBUG_LEVEL).Infof("Successfully added the bridge fdb entry %v", neigh)
	}
	return nil
}
...
func (iface *vxLanIface) DelFDB(ipAddress net.IP, hwAddr string) error {
	...
	if err != nil {
		...
		klog.V(log.DEBUG_LEVEL).Infof("Successfully deleted the bridge fdb entry %v", neigh)
	}
	return nil
}
