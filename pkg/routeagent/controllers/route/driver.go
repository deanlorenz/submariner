	...
	for _, iface := range hostInterfaces {
		...
		for i := range addrs {
			...
			if err != nil {
				...
				klog.V(log.DEBUG_LEVEL).Infof("Interface %q has %q address", iface.Name, ipAddr)
				...
				if clusterNetwork.Contains(address) {
					klog.V(log.DEBUG_LEVEL).Infof("Found CNI Interface %q that has IP %q from ClusterCIDR %q",
...
func toggleCNISpecificConfiguration(iface string) error {
	...
	if err != nil {
		...
		klog.V(log.DEBUG_LEVEL).Infof("Successfully configured rp_filter to loose mode(2) on cniInterface %q", iface)
	}
	return nil
}
