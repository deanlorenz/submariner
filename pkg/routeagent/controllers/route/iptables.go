...
func (r *Controller) createIPTableChains() error {
	...
	klog.V(log.DEBUG_LEVEL).Infof("Install/ensure %s chain exists", SmPostRoutingChain)
	...
	klog.V(log.DEBUG_LEVEL).Infof("Insert %s rule that has rules for inter-cluster traffic", SmPostRoutingChain)
	...
	klog.V(log.DEBUG_LEVEL).Infof("Install/ensure SUBMARINER-INPUT chain exists")
	...
	klog.V(log.DEBUG_LEVEL).Infof("Allow VxLAN incoming traffic in SUBMARINER-INPUT Chain")
	...
	klog.V(log.DEBUG_LEVEL).Infof("Insert rule to allow traffic over %s interface in FORWARDing Chain", VxLANIface)
	...
	if r.cniIface != nil {
		...
		klog.V(log.DEBUG_LEVEL).Infof("Installing rule for host network to remote cluster communication: %s", strings.Join(ruleSpec, " "))
...
func (r *Controller) programIptableRulesForInterClusterTraffic(remoteCidrBlock string) error {
	...
	for _, localClusterCidr := range r.localClusterCidr {
		...
		klog.V(log.DEBUG_LEVEL).Infof("Installing iptables rule for outgoing traffic: %s", strings.Join(ruleSpec, " "))
		if err = ipt.AppendUnique("nat", SmPostRoutingChain, ruleSpec...); err != nil {
		...
		klog.V(log.DEBUG_LEVEL).Infof("Installing iptables rule for incoming traffic: %s", strings.Join(ruleSpec, " "))
		if err = ipt.AppendUnique("nat", SmPostRoutingChain, ruleSpec...); err != nil {
			return fmt.Errorf("error appending iptables rule \"%s\": %v\n", strings.Join(ruleSpec, " "), err)
		}
	}
